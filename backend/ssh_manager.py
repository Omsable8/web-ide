...
class SSHManager:
    ...
    def run_code_with_input(self, code: str, language: str, input_data: str) -> dict:
        ...
        try:
            ...
            filename = f"temp_code_{timestamp}.{ext}"
            input_filename = f"temp_input_{timestamp}.txt"
            
            # Write code and input to files
            escaped_code = code.replace('"', '\\"').replace('$', '\\$')
            write_code_cmd = f'echo "{escaped_code}" > {filename}'
            
            escaped_input = input_data.replace('"', '\\"').replace('$', '\\$')
            write_input_cmd = f'echo "{escaped_input}" > {input_filename}'
            
            self.client.exec_command(write_code_cmd).wait()
            self.client.exec_command(write_input_cmd).wait()
            
            # Compile and run based on language
            if language.lower() == "python":
                exec_cmd = f"python3 {filename} < {input_filename}"
            elif language.lower() == "cpp":
                output_file = f"temp_output_{timestamp}"
                compile_cmd = f"g++ {filename} -o {output_file}"
                self.client.exec_command(compile_cmd).wait()
                exec_cmd = f"./{output_file} < {input_filename}"
            elif language.lower() == "java":
                exec_cmd = f"javac {filename} && java TempCode < {input_filename}"
            else:
                return {"success": False, "error": f"Unsupported language: {language}"}
            ...
            # Cleanup
            cleanup_cmd = f"rm -f {filename} {input_filename} temp_output_*"
            self.client.exec_command(cleanup_cmd)
