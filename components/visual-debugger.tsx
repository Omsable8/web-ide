...
export function VisualDebugger({
	...
	const getStructureType = (val: string) => {
		...
		if (trimmed.includes('[[') && trimmed.includes(']]')) return 'matrix'
		if (trimmed.includes('[') && trimmed.includes(']')) return 'array'
		if (trimmed.includes('{') && trimmed.includes('}')) return 'map'
		return 'primitive'
	}
	/**
 * Parses "[[1, 2], [3, 4]]" into [["1", "2"], ["3", "4"]]
 */
	// Update parseArray and parseMatrix to handle potential "int [3]" labels 
	// and memory addresses that might still leak in.
	const parseMatrix = (val: string) => {
		// Remove only the outermost [[ and ]]
		const content = val.trim().replace(/^\[\[/, '').replace(/\]\]$/, '');
		// Split by rows: ], [ 
	...
	const parseArray = (val: string) => {
		return val
			...
			.split(',')
			.map((s) => s.trim())
			// Filter out memory addresses if they leak as strings
			.filter((s) => !s.startsWith('0x') && s !== "")
	}
						...
						return (
							...
							>
								...
								<div className="p-4 rounded-xl bg-slate-900/90 border border-slate-700 shadow-xl backdrop-blur-sm">
									...
									{type === 'matrix' && (
										<div className="flex flex-col gap-2 p-2 bg-amber-500/5 rounded-xl border border-amber-500/20">
											{parseMatrix(value).map((row, rowIndex) => (
												<div key={`row-${rowIndex}`} className="flex gap-2">
													{row.map((item, colIndex) => (
														<motion.div
															key={`${name}-cell-${rowIndex}-${colIndex}`}
															layoutId={`${name}-cell-${rowIndex}-${colIndex}`}
															className="w-10 h-10 flex items-center justify-center bg-amber-500/10 border border-amber-500/30 rounded-lg text-amber-400 font-mono text-sm shadow-sm"
														>
															{item}
														</motion.div>
													))}
											...
											<div className="text-[10px] text-amber-500/50 font-mono mt-1 px-1">
												{parseMatrix(value).length} x {parseMatrix(value)[0]?.length || 0}
											</div>
										</div>
									)}
									...
									{type === 'map' && (
										<div className="flex flex-col gap-2 min-w-[200px]">
											{value
												...
												.split(/,(?![^\[]*\])/) // Split by comma not inside brackets
												.map((pair: string, i: number) => {
													// CLEANUP: If the string looks like "{first: 1, second: 1}", extract the values
													let displayKey = "";
													let displayVal = "";

													if (pair.includes('first:') && pair.includes('second:')) {
														const firstMatch = pair.match(/first:\s*([^,]+)/);
														const secondMatch = pair.match(/second:\s*([^}]+)/);
														displayKey = firstMatch ? firstMatch[1].trim() : "key";
														displayVal = secondMatch ? secondMatch[1].trim() : "val";
													} else {
														const separatorIndex = pair.search(/[:=]/);
														displayKey = pair.substring(0, separatorIndex).trim();
														displayVal = pair.substring(separatorIndex + 1).trim();
													}

													if (!displayKey) return null;

													return (
														<div key={i} className="flex items-center justify-between gap-4 px-3 py-1.5 bg-purple-500/10 border border-purple-500/30 rounded-lg">
															<span className="text-purple-300 font-mono text-xs font-bold">{displayKey}</span>
															<div className="h-px flex-1 bg-slate-800 mx-2" />
															<span className="text-emerald-400 font-mono text-xs">{displayVal}</span>
														</div>
													);
												})}
										</div>
